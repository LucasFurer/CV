...
%\definecolor{background1}{HTML}{3d5b81}
%\definecolor{background2}{HTML}{99c1da}
%\definecolor{col3}{HTML}{dffbfc}
%\definecolor{picBorderCol}{HTML}{293241}
%\definecolor{linkCol}{HTML}{ee6b4d}
...
\definecolor{linkCol}{HTML}{000000}
...
\begin{tikzpicture}[remember picture, overlay, shift={(current page.north west)}]
	...
	% Note: Points on the page will have positive X and negative Y values.
	
	% Draw a circle right at the top-left corner
	%\fill[red] (0,0) circle (50pt);
	
	% Place a node 3cm to the right and 4cm down from the top-left corner
	%\node[anchor=north west] at (4, -4) [draw, fill=white] {
	%  Top-Left Origin Node
	%};
	
	% Draw a diagonal line down and across the page
	%\draw[blue, thick] (1, -1) -- (10, -10);
% Top-Left: (0, 0)
% Bottom-Right for A4: (21, -29.7)
	...
	% --- summary ------------------------------------
	\node[anchor=north west, text width=12.5cm,align=left, text=white, font=\large] at (7.0, -4.6)
	{
A detail-oriented and enthusiastic MSc Computer Science graduate with a passion for physics-based simulations, computer graphics and high-performance computing. I am an eager learner who enjoys collaborating with others and exploring new technologies. Experienced with C++, Java, HLSL and python. 
	};
	...
	% --- section variables ------------------------------------
	\coordinate (sectStart) at ($(4.0,-9.3)$);


	\pgfmathsetmacro{\horGapSmall}{0.2} % space between text and the line
	\pgfmathsetmacro{\horGapLarge}{16.0} % space between line and dates
	\pgfmathsetmacro{\textWidth}{15.2} % width of the text

	\pgfmathsetmacro{\vertTextGap}{0.05} % spacing between text
	\pgfmathsetmacro{\vertSubGap}{0.2} % spacing between sub headers
		...
		at ($(eduStart) + (\horGapSmall,0)$)
	{
		...
		$\bullet$ \textbf{Notable courses:} Adv. Digital Image Processing, Category Theory, Geometry Processing\\
	};
		...
		at ($(eduEntryA.south west) + (0,-\vertTextGap)$)
	{
		...
		$\bullet$ \textbf{Notable courses:} Intro to Quantum Computing, Big Data Processing, Computer Graphics\\
	};
	\node[anchor=north east, align=left] 
	...
	{
		\textbf{Independent Game Development Project}, \textit{Software Engineer}\\
	...
	{
		\textbf{Blender plugin project}, \textit{CS4540 Geometric Data Processing university project}\\
	...
	{
		\textbf{C++}\\
		Libraries: Boost, Eigen, Fastor, GLM, OpenGL, GLFW, FFTW, Vulkan
	};
	\node[anchor=north east, align=left, font=\normalsize] 
...
%	};
	...
	{
		\textbf{C, Python, C\#, Java, HLSL, CUDA, Unity}\\
...
%	};
	...
	{
		\textbf{CMake, Docker, Linux, Bash, Git, Blender, Markdown, LaTeX}\\	
	};
	\node[anchor=north east, align=left, font=\normalsize] 
